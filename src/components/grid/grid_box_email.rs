...
pub struct GridBoxEmail;

impl Component for GridBoxEmail {
  type Message = ();
  type Properties = ();
          ...
          <div class="font-sans text-lg lg:text-3xl max-w-96 font-bold z-10">{"Quer começar um projeto juntos?"}</div>
            <div class="mt-5 relative">
              <div class="absolute -bottom-5 right-0 block ">
                <div style="width:400px;height:200px;overflow:hidden;margin:0 auto;outline:none" title="" role="button" aria-label="animation" tabindex="0"></div>
              </div>
              <MagicButton>
